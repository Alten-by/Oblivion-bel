...
## Дадаткова (Крок 6, порцыя 2) - выяўлены "Real Barenziah" (4 вялікія
кнігі), выпраўленне тэгаў drop-cap

- **АДКЛАДЗЕНЫ 4 запісы** (застаюцца пустымі, вернуцца ў наступнай
  экстракцыі): `Book2CommonRealBarenziahV1-V4` (~20-25 тыс. сімвалаў
  кожны). Ні `bel`, ні `bel_alt` не супадаюць з `en` па аб'ёме --
  `bel` абрываецца на паўслове (сапраўды незакончаны пераклад, не
  проста іншая разбіўка), `bel_alt` для V4 увогуле абрэзаны да
  636 сімвалаў з 25107. Гэтыя 4 кнігі патрабуюць альбо поўнага
  ручнога перакладу (вельмі працаёмка), альбо асобнага рашэння
  карыстальніка. Пры далейшай працы звярнуць на іх увагу асобна.
- **Шаблон drop-cap тэга**: у кнігах пачатак асноўнага тэксту часта
  мае `<IMG ID="BOOK/FANCY_FONT/X_NxM.DDS".../>` -- дэкаратыўная
  першая літара. У `en` наступнае за тэгам слова заўсёды губляе сваю
  першую літару (яна "намалявана" малюнкам). Пры перакладзе трэба
  захоўваць гэты ж шаблон: тэг застаецца на месцы, а першая літара
  перакладзенага слова апускаецца (нават калі яна не супадае з
  літарай на малюнку) -- патрэбна ўважліва правяраць гэта пры
  выбары/пабудове перакладу, чарнавікі часта губляюць гэты тэг.
- Выпраўлена русізмы: "маги" -> "магі" (ReportDisasterIonith),
  "Травництва" -> "Травніцтва" (непаслядоўнае напісанне ў адным
  дакуменце -- у астатнім тэксце ўжо было правільна "травніцтва").
- `IstunondsCosmology` (насамрэч верш "A Less Rude Song"): выпадковы
  тэг `<font face=1>` пасярод слова "singing" у `en` -- пры
  перакладзе ўстаўлены аналагічны тэг усярэдзіне адпаведнага слова ў
  беларускім тэксце (стылёвы артэфакт зыходнага тэксту, не нясе
  сэнсавай нагрузкі).

## Дадаткова (Крок 6, порцыя 3) - "Real Barenziah" V1-V5 адноўлены з
беларускай лакалізацыі Skyrim

- Паводле явнай просьбы карыстальніка ("у перакладзе скайрыма ёсць
  некаторыя кнігі. Можаш іх пашукаць каб меньш перакладаць") усе 5
  тамоў "Real Barenziah" (адкладзеныя V1-V4 з порцыі 2, плюс V5)
  адноўлены з ужо гатовай беларускай лакалізацыі Skyrim
  (`skyrim bel/Strings/skyrim_english.dlstrings`), дзе гэтая ж кніга
  прысутнічае як унутрыгульнявы агульны лор-тэкст.
- **Канвертацыя тарашкевіцы ў наркамаўку**: лакалізацыя Skyrim
  выкарыстоўвае класічны правапіс (тарашкевіцу), а гэты праект
  патрабуе афіцыйны наркамаўскі правапіс. Пабудаваны і эмпірычна
  адладжаны скрыпт-канвертар з правіламі:
  - с/з + ь + зычная -> без ь (АЛЕ не для ц/л, гл. выключэнні
    "бацька", "здольнасць", "вольны" -- яны сапраўды захоўваюць ь);
  - дз + ь + зычная -> дз;
  - ьн -> н (аддзеяслоўны суфікс -ьне/-ьня), АЛЕ не пасля л
    (выключэнне для "вольны"/"здольнасць"-тыпу слоў);
  - "цьц" -> "цц" (гемінаты тыпу "жыцьцё" -> "жыццё");
  - зь + я/е/ё/ю -> з' + галосная (апострафны канвент,
    "зьявіўся" -> "з'явіўся");
  - выключэнні-слоўнік для няпэўных займеннікаў на "-сьці", якія
    захоўваюць ь: штосьці, дзесьці, чамусьці, кудысьці, якісьці,
    якойсьці (не блытаць з інфінітывамі тыпу "прайсці");
  - ьдз -> дз ("стагодьдзе" -> "стагоддзе");
  - асобны прыназоўнік "зь" -> "з";
  - невялікі слоўнік запазычанняў: філязоф -> філасаф,
    клясы -> класы, плян -> план.
- **V5**: у зыходным `en` тэкст пашкоджаны/падвойна экранаваны --
  замест тэга `<lf>` літаральныя двухсімвальныя `\n`, замест простых
  двукоссяў `\"` (з дзіўным прабелам, напр. `\"center \"`), па ўсім
  запісе (340 `\n` і 310 `\"`). Бо `en` нельга змяняць, а
  `validate.py` параўноўвае тэгі як дакладныя падрадкі, беларускі
  тэкст пабудаваны з такім жа экранаваннем, а не звычайнымі `<lf>`.
- **Дэкаратыўныя маркеры сцэн** (`<div align="center">***<div
  align="left">`) у `en` НЕ ўзноўлены ў беларускім тэксце -- пасля
  некалькіх няўдалых спроб прапарцыйнай устаўкі (падвойнае
  экранаванне, збітыя лічыльнікі `<lf>`) прынята рашэнне гэта
  апусціць як чыста касметычны прабел, згодна з ужо ўсталяваным у
  праекце прынцыпам дапушчальных фарматавальных разыходжанняў у вельмі
  доўгіх кнігах (гл. FundamentsofAlchemy). `validate.py` паказвае гэта
  як вядомае/дапушчальнае адхіленне для ўсіх 5 тамоў.
- Выпраўлена русізмы ў серыі альманахаў "2920" і іншых кнігах гэтай
  порцыі: "куртизанка" -> "куртызанка" (Valuable2920v11a), "Сота Сил"
  -> "Сота Сіл" (уласнае імя, Valuable2920v11a), "стропил" ->
  "страпіл" (Valuable2920v12a), "маги" -> "магі" (Valuable2920v3a).
